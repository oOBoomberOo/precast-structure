...
// Atlas-safe sample: UV offsets can hit transparent padding between sprites.
// Fall back to the center texel so connected blocks never tear open.
...
void main() {
    // GameTime is 0..1 per Minecraft day — convert to seconds for readable motion.
    ...
    // Rolling UV jitter / glitch tears (visual only — never discard from these).
    ...
    // Chromatic aberration with atlas-safe channel picks.
    ...
    // Keep block detail; lift and cool-tint.
    ...
    // Tear rows flash as brighter glitch bands instead of punching holes.
    tinted = mix(tinted, tinted * vec3(1.15, 1.35, 1.55) + hologramCyan * 0.25, tear);

    // Gentle flicker + mild dropout (brightness only).
    ...
    // Coverage always comes from the real block texel so faces stay merged.
