...
/**
 * GUI ghost items: vanilla {@link GuiGraphics#setColor} / {@code setShaderColor} do not tint {@link
 * GuiGraphics#renderItem} (item draws ignore shader color). This path multiplies vertex alpha and
 * forces translucent entity layers so the icon actually blends.
 */
final class GhostItemRenderer {
  private GhostItemRenderer() {}

  /**
   * Invoked via {@code Class.forName} from {@link StructurePrinterScreen}, not a direct call. Menu
   * registration uses {@code StructurePrinterScreen::new}, which loads that class early; any
  ...
  public static void render(GuiGraphics guiGraphics, ItemStack stack, int x, int y, float alpha) {
    ...
    MultiBufferSource ghostBuffers =
        renderType ->
            new AlphaTintVertexConsumer(
                bufferSource.getBuffer(translucentLayer(renderType)), alpha);
  ...
  private static final class AlphaTintVertexConsumer implements VertexConsumer {
    ...
    private int tint(int argb) {
      ...
      return FastColor.ARGB32.color(
          a, FastColor.ARGB32.red(argb), FastColor.ARGB32.green(argb), FastColor.ARGB32.blue(argb));
    }
  }
}
