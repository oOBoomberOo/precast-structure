...
public class StructurePrinterMenu extends AbstractContainerMenu {
    ...
    public StructurePrinterMenu(int containerId, Inventory inventory, FriendlyByteBuf buf) {
        this(containerId, inventory, new SimpleContainer(CONTAINER_SLOT_COUNT), new SimpleContainerData(StructurePrinterBlockEntity.DATA_COUNT), ContainerLevelAccess.create(inventory.player.level(), buf.readBlockPos()), inventory.player.level().registryAccess());
    }

    public StructurePrinterMenu(int containerId, Inventory inventory, StructurePrinterBlockEntity blockEntity) {
        this(containerId, inventory, blockEntity, blockEntity.getProgressData(), ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), blockEntity.getLevel().registryAccess());
    }

    private StructurePrinterMenu(int containerId, Inventory inventory, Container container, ContainerData progressData, ContainerLevelAccess access, HolderLookup.Provider registries) {
        ...
        this.addSlot(new Slot(container, StructurePrinterBlockEntity.BLUEPRINT_SLOT, BLUEPRINT_SLOT_X, BLUEPRINT_SLOT_Y) {
            @Override
    ...
    public void refreshLayout() {
        ...
        for (int i = 0; i < StructurePrinterBlockEntity.MATERIAL_SLOT_COUNT; i++) {
            ...
            if (i >= firstVisible && i < lastVisible && isMaterialSlotPresent(i)) {
                ...
                SlotLayout.set(slot, MATERIAL_SLOT_X + column * SLOT_SPACING, MATERIAL_SLOT_Y + row * SLOT_SPACING);
            } else {
    ...
    private boolean isMaterialSlotPresent(int materialIndex) {
        return getMaterialRequirement(materialIndex) != null
            || !container.getItem(StructurePrinterBlockEntity.FIRST_MATERIAL_SLOT + materialIndex).isEmpty();
    }
    ...
    public MaterialRequirement getMaterialRequirement(int materialIndex) {
        ...
        if (materialIndex < 0 || materialIndex >= requirements.size() || materialIndex >= StructurePrinterBlockEntity.MATERIAL_SLOT_COUNT) {
            return null;
        }
    ...
    public int getActiveMaterialSlotCount() {
        return Math.min(getMaterialRequirements().size(), StructurePrinterBlockEntity.MATERIAL_SLOT_COUNT);
    }
    ...
    private int findMaterialSlotFor(ItemStack stack) {
        List<MaterialRequirement> requirements = getMaterialRequirements();
        for (int i = 0; i < Math.min(requirements.size(), StructurePrinterBlockEntity.MATERIAL_SLOT_COUNT); i++) {
