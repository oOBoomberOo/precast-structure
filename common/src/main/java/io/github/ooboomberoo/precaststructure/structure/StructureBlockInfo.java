...
public record StructureBlockInfo(BlockPos offset, BlockState state, @Nullable CompoundTag nbt) {
  public static final Codec<StructureBlockInfo> CODEC =
          ...
          instance ->
              instance
                  .group(
                      ...
                      CompoundTag.CODEC
                          .optionalFieldOf("nbt")
                          .forGetter(block -> Optional.ofNullable(block.nbt())))
                  .apply(
                      instance,
                      (x, y, z, state, nbt) ->
                          new StructureBlockInfo(new BlockPos(x, y, z), state, nbt.orElse(null))));

  private static final StreamCodec<ByteBuf, CompoundTag> NULLABLE_NBT =
      ByteBufCodecs.OPTIONAL_COMPOUND_TAG.map(
          optional -> optional.orElse(null), nbt -> Optional.ofNullable(nbt));

  public static final StreamCodec<ByteBuf, StructureBlockInfo> STREAM_CODEC =
