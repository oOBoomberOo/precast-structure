...
/**
 * Client-only Create soft-compat. Kinetic relays (shafts/cogs) bake empty quads unless ModelData is
 * marked virtual; brackets are a separate static mesh from BE NBT. Machines like millstones/mixers
 * keep their cogs in the block-entity renderer, so those are drawn via a temporary BE on Create's
 * SchematicLevel (avoids Flywheel skipping).
 */
public final class CreateCompatClient {
  ...
  @Nullable private static final Constructor<?> SCHEMATIC_LEVEL_CTOR;
  @Nullable private static final Method MARK_VIRTUAL;

  @Nullable private static Level cachedVirtualLevel;
  @Nullable private static Level cachedVirtualLevelSource;
    ...
    if (CreateCompat.isLoaded()) {
      ...
      try {
        Class<?> schematicLevel =
            Class.forName("net.createmod.catnip.levelWrappers.SchematicLevel");
