 ...
 *
 * <p>Iris cannot run custom core shaders inside an active shader pack's gbuffer pipeline (meshes
 * using those programs are dropped). When a pack is active we keep holograms visible by drawing
 * with vanilla solid/translucent programs and applying the hologram look on the CPU via {@link
 * HologramStyleVertexConsumer}.
 *
 * <p>Deferred packs also composite over anything drawn during the opaque/entity phase, so overlays
 * must flush after translucent geometry when a pack is active.
 */
public final class ShaderCompat {
  ...
  /**
   * Deferred shader packs overwrite translucent draws made during the entity phase. Flush holograms
   * after translucent/particles when a pack is active.
   */
