 ...
 * Timing matches scanning ({@code max(80, height * 16)}), but the plane moves bottom → top. Real
 * blocks are written only when the animation completes.
 */
public final class StructureDeployment {
  ...
  public StructureDeployment(
    ...
    for (StructureBlockInfo block : blueprint.blocks()) {
      BlockPos worldPos =
          origin.offset(StructurePlacement.transformOffset(block.offset(), blueprint, this.facing));
  ...
  public static StructureDeployment create(
      BlockPos origin, Direction facing, StructureBlueprint blueprint, long gameTime) {
    int height = Math.max(1, blueprint.size().getY());
    return new StructureDeployment(
        UUID.randomUUID(), origin, facing, blueprint, gameTime, durationForHeight(height));
  }
  ...
  public void placeAll(Level level) {
    ...
    if (level instanceof net.minecraft.server.level.ServerLevel serverLevel) {
      for (StructureBlockInfo block : blueprint.blocks()) {
        BlockPos worldPos =
            origin.offset(StructurePlacement.transformOffset(block.offset(), blueprint, facing));
