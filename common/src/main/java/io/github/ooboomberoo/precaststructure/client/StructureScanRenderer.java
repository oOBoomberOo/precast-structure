...
/**
 * Scan ghosts clipped precisely to the moving scan plane: solid block mesh below the plane,
 * textured hologram mesh above it.
 */
public final class StructureScanRenderer {
  ...
  public static void render(PoseStack poseStack, Vec3 cameraPosition, float partialTick) {
    ...
    for (StructureScannerBlockEntity scanner : StructureScannerBlockEntity.clientActiveScans()) {
      if (scanner.isScanning()
          && scanner.getGhostBlueprint() != null
          && !scanner.getGhostBlueprint().blocks().isEmpty()) {
  ...
  private static void renderSolids(
    ...
    for (StructureBlockInfo block : ghosts.blocks()) {
      BlockPos worldPos =
          StructurePlacement.localToScanWorld(origin, scanSize, scannerFacing, block.offset());
  ...
  private static void collectHologramParts(
      StructureScannerBlockEntity scanner, float partialTick, List<Part> out) {
    StructureBlueprint ghosts = scanner.getGhostBlueprint();
    if (ghosts == null || ghosts.blocks().isEmpty()) {
      return;
    }
  ...
  private static @Nullable net.minecraft.nbt.CompoundTag scanNbt(
      StructureBlockInfo block, Direction scannerFacing, @Nullable Level level) {
