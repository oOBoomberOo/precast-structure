...
/**
 * Applies {@link HologramEffectMath} on the CPU so Iris/Oculus shader packs still get the hologram
 * look while using vanilla programs Iris can remap.
 */
public final class HologramStyleVertexConsumer implements VertexConsumer {
  ...
  public HologramStyleVertexConsumer(
      VertexConsumer delegate, float cameraX, float cameraY, float cameraZ, float time) {
  ...
  public void addVertex(
    ...
    lastZ = z;
    delegate.addVertex(
        x, y, z, style(color), u, v, packedOverlay, packedLight, normalX, normalY, normalZ);
  }
  ...
  private int style(int argb) {
    return HologramEffectMath.styleColor(
        argb, lastX + cameraX, lastY + cameraY, lastZ + cameraZ, time);
  }
