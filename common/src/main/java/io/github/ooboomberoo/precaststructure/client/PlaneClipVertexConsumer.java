...
/**
 * {@link VertexConsumer} that clips submitted geometry to a horizontal wipe plane, used for
 * hologram deploy/scan reveal effects.
 *
 * <p>Two clip heights may be supplied:
 *
 * <ul>
 *   <li>{@code modelClipY} — Y in block-model space, applied to baked quads ({@link #putBulkData}).
 *   <li>{@code outputClipY} — Y in pose-output space, applied to already-transformed vertices
 *       ({@link #addVertex(float, float, float, int, float, float, int, int, float, float,
 *       float)}). Often {@code blockPose.transformPosition(0, modelClipY, 0).y}.
 * </ul>
 *
...
public final class PlaneClipVertexConsumer implements VertexConsumer {
   ...
   * @param outputClipY wipe Y in the same space as already-transformed BER verts (typically {@code
   *     blockPose.transformPosition(0, modelClipY, 0).y})
   */
  public PlaneClipVertexConsumer(
      VertexConsumer delegate, float modelClipY, float outputClipY, boolean keepBelow) {
  ...
  public void putBulkData(
    ...
    Vector3f transformedNormal =
        pose.transformNormal(normal.getX(), normal.getY(), normal.getZ(), new Vector3f());
  ...
  /**
   * Accepts a fully attributed vertex in pose-output space; every four vertices form a quad that is
   * clipped at {@code outputClipY}.
   */
  @Override
  public void addVertex(
      ...
      float normalY,
      float normalZ) {
    input[oneshotCount].set(
        x, y, z, color, u, v, packedLight, packedOverlay, normalX, normalY, normalZ);
    oneshotCount++;
    if (oneshotCount == 4) {
  ...
  /**
   * Emits the clipped polygon as {@link VertexFormat.Mode#QUADS} primitives. When {@code matrix} is
   * non-null, vertices are transformed before emit; when null, they are already in output space.
   */
  ...
  private static int clipQuad(
      ClipVert[] quad, float clipY, boolean keepBelow, ClipVert[] out, ClipVert[] scratch) {
    for (int i = 0; i < 4; i++) {
  ...
  private static int clipAgainstPlane(
      ClipVert[] in, int inCount, ClipVert[] out, float clipY, boolean keepBelow) {
    int outCount = 0;
  ...
  private static boolean isInside(float y, float clipY, boolean keepBelow) {
    return keepBelow
        ? y <= clipY + HologramRenderSystem.CLIP_EPSILON
        : y >= clipY - HologramRenderSystem.CLIP_EPSILON;
  }
