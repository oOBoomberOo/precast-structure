 ...
 * them onto {@code entity_cutout} / {@link DefaultVertexFormat#NEW_ENTITY}. Vertex format alone is
 * not a reliable signal for layer choice under that remapping.
 *
 * <p><b>True BER meshes</b> (chest / bed / shulker) keep entity atlas layers via {@link
 * #resolveEntityBer(boolean)}.
 */
public final class HologramLayerPolicy {
  ...
  /**
   * Whether {@code format} matches {@link DefaultVertexFormat#NEW_ENTITY}. Block models remapped by
   * Veil may also report this format, so callers must not use this alone to choose between block
   * and entity hologram layers.
   */
