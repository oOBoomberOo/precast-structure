...
/**
 * Renders a temporary block entity through its vanilla BER (chests, beds, shulkers, signs, …) for
 * hologram / solid previews. Callers must supply a {@link MultiBufferSource} that preserves entity
 * vertex formats — remapping them onto the block-atlas hologram layer breaks UVs.
 *
 * <p>Beds are special: vanilla only attaches a BE to the head, but {@link
 * net.minecraft.client.renderer.blockentity.BedRenderer} draws a single half per BE based on {@code
 * PART}. Preview entities are created for both halves so the foot piece is not missing from
 * holograms.
 *
...
public final class BlockEntityPreviewRenderer {
  ...
  /**
   * True when the world mesh comes from the BER rather than a baked block model (chests, shulkers,
   * beds, signs, skulls, banners, …).
   */
  ...
  /**
   * Builds a transient BE for preview. Beds construct {@link BedBlockEntity} for both head and foot
   * — {@link BedBlock#newBlockEntity} returns null for the foot.
   */
