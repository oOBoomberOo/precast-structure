 ...
 * storage coordinates but render through a {@code Pose3d}. World holograms must apply that
...
public final class SableCompatClient {
     ...
     * Equals the pose rotation point when {@code poseAnchor} is in a sub-level; otherwise zero.
     */
    ...
    /**
     * Applies camera-relative translation, and when {@code poseAnchor} sits in a Sable
     * sub-level, the contraption pose {@code T(position - camera) * R * S} (no rotation-point
     * bake). Pair with {@link #plotOrigin} when converting plot-storage coordinates.
