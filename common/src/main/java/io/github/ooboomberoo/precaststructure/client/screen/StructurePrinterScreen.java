...
public class StructurePrinterScreen extends AbstractContainerScreen<StructurePrinterMenu> {
  private static final ResourceLocation TEXTURE =
      ResourceLocation.fromNamespaceAndPath(
          PrecastStructureMod.MOD_ID, "textures/gui/structure_printer.png");
  ...
  protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
    ...
    if (this.menu.canScroll()) {
      ...
      guiGraphics.fill(
          trackX,
          trackY,
          trackX + StructurePrinterMenu.SCROLLBAR_WIDTH,
          trackY + StructurePrinterMenu.SCROLLBAR_HEIGHT,
          0xFF000000);
      ...
      guiGraphics.blit(
          ...
          SCROLLBAR_UV_X,
          SCROLLBAR_UV_Y,
          StructurePrinterMenu.SCROLLBAR_WIDTH,
  ...
  protected void renderLabels(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    guiGraphics.drawString(
        this.font, this.title, this.titleLabelX, this.titleLabelY, LABEL_COLOR, false);
    guiGraphics.drawString(
        this.font, this.playerInventoryTitle, 8, this.inventoryLabelY, LABEL_COLOR, false);
  }
  ...
  protected void renderSlot(GuiGraphics guiGraphics, Slot slot) {
    ...
    if (slot.index == StructurePrinterBlockEntity.OUTPUT_SLOT) {
      renderGhostItem(
          guiGraphics, new ItemStack(ModItems.PRECAST_STRUCTURE.get()), slot.x, slot.y, null);
      return;
    }

    if (slot.index < StructurePrinterBlockEntity.FIRST_MATERIAL_SLOT
        || slot.index >= StructurePrinterBlockEntity.OUTPUT_SLOT) {
      return;
    }
    ...
    ItemStack ghost =
        new ItemStack(
            requirement.item(),
            Mth.clamp(requirement.amount(), 1, requirement.item().getDefaultMaxStackSize()));
  ...
  private boolean isOverMaterialsArea(double mouseX, double mouseY) {
    ...
    int w =
        StructurePrinterMenu.MATERIAL_COLUMNS * StructurePrinterMenu.SLOT_SPACING
            + StructurePrinterMenu.SCROLLBAR_WIDTH
            + 4;
  ...
  private boolean isOverScrollbar(double mouseX, double mouseY) {
    ...
    return mouseX >= x
        && mouseX < x + StructurePrinterMenu.SCROLLBAR_WIDTH
        && mouseY >= y
        && mouseY < y + StructurePrinterMenu.SCROLLBAR_HEIGHT;
  }

  private void renderGhostItem(
      GuiGraphics guiGraphics, ItemStack ghost, int x, int y, String count) {
    ...
    try {
      Class.forName("io.github.ooboomberoo.precaststructure.client.screen.GhostItemRenderer")
          .getDeclaredMethod(
              "render", GuiGraphics.class, ItemStack.class, int.class, int.class, float.class)
    ...
    if (count != null) {
      ...
      guiGraphics.drawString(
          this.font, count, x + 19 - this.font.width(count), y + 9, GHOST_COUNT_COLOR, true);
