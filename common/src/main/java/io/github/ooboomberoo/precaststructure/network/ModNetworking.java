...
public final class ModNetworking {
    ...
    public static final ResourceLocation DEPLOY_ADD = ResourceLocation.fromNamespaceAndPath(PrecastStructureMod.MOD_ID, "deploy_add");
    public static final ResourceLocation DEPLOY_REMOVE = ResourceLocation.fromNamespaceAndPath(PrecastStructureMod.MOD_ID, "deploy_remove");
    ...
    public static void registerClient() {
        NetworkManager.registerReceiver(NetworkManager.serverToClient(), DEPLOY_ADD, (buf, context) -> {
            StructureDeployment deployment = readDeployAdd(buf);
            context.queue(() -> StructureDeploymentManager.clientAdd(deployment));
        });
        NetworkManager.registerReceiver(NetworkManager.serverToClient(), DEPLOY_REMOVE, (buf, context) -> {
    ...
    private static RegistryFriendlyByteBuf writeDeployAdd(StructureDeployment deployment, RegistryAccess registryAccess) {
    ...
    private static StructureDeployment readDeployAdd(RegistryFriendlyByteBuf buf) {
        ...
        StructureBlueprint blueprint = StructureBlueprint.load(blueprintTag != null ? blueprintTag : new CompoundTag(), buf.registryAccess())
