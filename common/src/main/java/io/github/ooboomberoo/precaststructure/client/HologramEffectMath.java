...
/**
 * Shared hologram look used by the core fragment shader and the Iris-safe vertex path.
 * Mirrors {@code scan_hologram.fsh} so shader-pack users still get scanlines / sweeps / glitches.
 */
public final class HologramEffectMath {
    ...
    public static int styleColor(int argb, float worldX, float worldY, float worldZ, float time) {
        ...
        float noise = hash(Mth.floor(worldX * 24.0F) + Mth.floor(time * 12.0F), Mth.floor(worldZ * 24.0F));
        tr += hcR * noise * 0.06F;
        tg += hcG * noise * 0.06F;
        tb += hcB * noise * 0.06F;
